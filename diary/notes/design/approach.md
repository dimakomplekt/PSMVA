...
1) Коллить все конструкторы и настройку UI посылая нужные функции внутрь current_state.start()
2) Коллить update() всех используемых в стейте UI в current_state.update()
3) Коллить render() всех используемых в стейте UI в current_sate.render()
4) Коллить все деструкторы UI на current_tstate.exit()

Таким образом получается всегда держать под контролем структуру приложения, чистенько верстать в init и рендерить навёрстанное в одной функции, всегда имея свежую дату и флаги под рендер. При этом память будет динамически выделяться лишь под нужный стейт. Во избежание утечек лучше придерживаться правила - дал конструктор - дал деструктор, потом - всё остальное.